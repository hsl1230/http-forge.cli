...
- **`@http-forge/core` upgraded to ^0.6.28** — inherits all core fixes: request body allowed for all HTTP methods, live-reload TLS certificates, HTML report body truncation, Postman import hardening, file-watcher rewrite, and reduced result file size.

### Fixed

- **Request body sent for all HTTP methods** — via `@http-forge/core` update, the `FetchHttpClient` no longer strips the body for GET and HEAD requests. CLI-executed requests (including MCP `run_request`) now pass the body through for any HTTP method, matching RFC 9110.
